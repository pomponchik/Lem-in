...
static void        connect_link(t_graph **graph, int first_link, int second_link)
{
    if (!(*graph)[first_link].adjacency)
        ...
        ft_lstadd(&(((*graph)[first_link]).adjacency), ft_lstnew_no_copy(&((*graph)[second_link]), sizeof(t_graph)));
    if (!(*graph)[second_link].adjacency)
        (*graph)[second_link].adjacency = ft_lstnew_no_copy(&((*graph)[first_link]), sizeof(t_graph));
    else
        ft_lstadd(&(((*graph)[second_link]).adjacency), ft_lstnew_no_copy(&((*graph)[first_link]), sizeof(t_graph)));
}
